...
pub trait Poseidon2: PrimeField64 {
    ...
    fn poseidon2(input: [Self; WIDTH]) -> [Self; WIDTH] {
        ...
        for r in 0..ROUND_P {
            // t_0 = x_0 + c_0^i
            current_state[0] += Self::from_canonical_u64(Self::RC_MID[r]);
            // t_1 = t_0^7
            current_state[0] = Self::sbox_monomial(current_state[0]);
            // M_I * t_1
            Self::matmul_internal(&mut current_state, &Self::MAT_DIAG_M_1);
        }
    ...
    fn constant_layer(state: &mut [Self; WIDTH], round_ctr: usize) {
        for i in 0..WIDTH {
            let round_constant = Self::RC[round_ctr][i];
            unsafe {
    ...
    {
        for i in 0..WIDTH {
            let round_constant = Self::Extension::from_canonical_u64(Self::RC[rc_index][i]);
        ...
        Self: RichField + Extendable<D>,
    {
        ...
        for i in 0..WIDTH {
            let round_constant = Self::Extension::from_canonical_u64(Self::MAT_DIAG_M_1[i] - 1);
            let round_constant = builder.constant_extension(round_constant);
...
impl Poseidon2Hash {
    pub fn permute_targets<F: RichField + Poseidon2, const D: usize>(
        inputs: &[Target],
        ...
        F: RichField + Extendable<D>,
    {
        ...
        let swap = builder.zero();
        builder.connect(swap, swap_wire);
        ...
        core::array::from_fn(|i| Target::wire(gate, Poseidon2Gate::<F, D>::wire_output(i)))
    }

    pub fn permute_targets_mut<F: RichField + Poseidon2, const D: usize>(
        inputs: &mut [Target],
        builder: &mut CircuitBuilder<F, D>,
    ) where
        F: RichField + Extendable<D>,
    {
        ...
        for i in 0..WIDTH {
            inputs[i] = Target::wire(gate, Poseidon2Gate::<F, D>::wire_output(i))
        }
    }
}
