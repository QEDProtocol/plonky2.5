...
pub trait CircuitBuilderB32<F: RichField + Extendable<D>, const D: usize> {
    ...
    fn rsh_u64(&mut self, x: &[U32Target; 2], n: u8) -> [U32Target; 2];
    ...
    for CircuitBuilder<F, D>
{
    ...
    fn lsh_u64(&mut self, x: &[U32Target; 2], n: u8) -> [U32Target; 2] {
        ...
        // let (lo, hi) = if n < 32 { (x[0], x[1]) } else { (x[1], x[0]) };
        let (lo, hi) = (x[0], x[1]);

        if n < 32 {
            let power_of_two = self.constant_u32(0x1 << (n % 32));
            let (lo0, hi0) = self.mul_u32(lo, power_of_two);
            let (lo1, _hi1) = self.mul_u32(hi, power_of_two);
            // xxxxxxxx0000,     yyyyyyyy0000
            //   hi0|lo0           hi1|lo1
            //    n|32-n            n|32-n
            [lo0, self.add_u32(hi0, lo1).0]
        } else {
            let power_of_two = self.constant_u32(0x1 << (n % 32));
            let (lo0, _hi0) = self.mul_u32(lo, power_of_two);
            // xxxxxxxx0000,     yyyyyyyy0000
            //   hi0|lo0           hi1|lo1
            //    n|32-n            n|32-n
            [U32Target(self.zero()), lo0]
        }
    }
    ...
    fn rsh_u64(&mut self, x: &[U32Target; 2], n: u8) -> [U32Target; 2] {
        ...
        if n < 32 {
            let power_of_two = self.constant_u32(0x1 << (32 - (n % 32)));
            let (_lo0, hi0) = self.mul_u32(lo, power_of_two);
            let (lo1, hi1) = self.mul_u32(hi, power_of_two);
            // xxxxxxxx0000,     yyyyyyyy0000
            //   hi0|lo0           hi1|lo1
            //   32-n|n            32-n|n
            [self.add_u32(lo1, hi0).0, hi1]
        } else {
            let power_of_two = self.constant_u32(0x1 << (32 - (n % 32)));
            let (_, hi1) = self.mul_u32(hi, power_of_two);
            // xxxxxxxx0000,     yyyyyyyy0000
            //   hi0|lo0           hi1|lo1
            //   32-n|n            32-n|n
            [hi1, U32Target(self.zero())]
        }
    }
}
...
mod tests {
    ...
    fn test_lsh_u64() {
        ...
        let tests = [
            (0x0u128, 0x0u128),
            (0x01234567u128, 0x01234567u128),
            (0x23456701u128, 0x23456701u128),
            (0x45670123u128, 0x45670123u128),
        ];
        ...
        let config = CircuitConfig::standard_recursion_config();
        let mut builder = CircuitBuilder::<F, D>::new(config);
        let input = builder.add_virtual_u32_targets(2);
        let expected = builder.add_virtual_u32_targets(2);
        let output = builder.lsh_u64(&[input[0], input[1]], 2);
        builder.connect_u64(output, [expected[0], expected[1]]);
        builder.register_public_inputs(&[expected[0].0, expected[1].0]);
        let data = builder.build::<C>();
    ...
    fn test_rsh_u64() {
        ...
        let input = builder.add_virtual_u32_targets(2);
        let expected = builder.add_virtual_u32_targets(2);
        let output = builder.rsh_u64(&[input[0], input[1]], 2);
        builder.connect_u64(output, [expected[0], expected[1]]);
        builder.register_public_inputs(&[expected[0].0, expected[1].0]);
        let data = builder.build::<C>();
